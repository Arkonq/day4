...
namespace WebApplication3.Controllers
{
	public class StudentsController : Controller
	{
		...
		public ActionResult Details(int? id)
		{
			...
			Student student = db.Students.Include(c => c.Courses).FirstOrDefault(x => x.Id == id);
			if (student == null)
			{
				return HttpNotFound();
			}
		...
		public ActionResult Enroll(StudentCoursesViewModel model)
		{
			...
			{
				if (student.Courses.Exists(c => c.Id == course.Id)) // Если элемент уже существует, и его пытаются добавить
